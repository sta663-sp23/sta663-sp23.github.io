...
```{python}
#| code-line-numbers: "|2"
def fit(b, loss, λ=0., n=200, lr=0.001, X=X, y=y, model=model):
...
```{python}
b = jax.random.normal(key, (X.shape[1],1))
b_hat, losses = fit(b, reg_loss, n=10)
```
:::
...
```{python}
b = jax.random.normal(key, (X.shape[1],1))
b_hat, losses = fit(b, ridge_loss, λ=10., n=10)
```
:::
...
```{python}
b = jax.random.normal(key, (X.shape[1],1))
b_hat, losses = fit(b, lasso_loss, λ=10., n=10)
```
:::
...
#| error: true
fit_jit = jax.jit(fit)
b_hat, losses = fit_jit(b, reg_loss, λ=10., n=10)
```
:::
...
::: {.small}
```{python}
def fit_simple(b, loss, λ=0., n=200, lr=0.001, X=X, y=y, model=model):
  grad = jax.grad(loss)
...
::: {.small}
```{python}
%timeit b_hat = fit_simple(b, reg_loss, n=50)
```

```{python}
%timeit b_hat_jit = fit_jit(b, reg_loss, n=50)
```

```{python}
%timeit b_hat_jit = fit_jit(b, reg_loss, n=50).block_until_ready()
```
:::
